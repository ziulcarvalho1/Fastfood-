...
shinyServer(
  function(input,output){
    ...
    output$mytempmin <- renderText(paste("You selected the minimum temperature as: ",input$tempmin))
    output$mytempmed <- renderText(paste("You selected the medium  temperature as: ",input$tempmed))
    output$mytempmax <- renderText(paste("You selected the maximum temperature as: ",input$tempmax))
    output$myprecip  <- renderText(paste("You selected the level of rain       as: ",input$precip))
    output$mysun     <- renderText(paste("You selected the level of sun exposition as: ",input$sun))
    ...
    inputData <- reactive({
        ...
        value = (c(input$precip, input$tempmax, input$tempmin,
                             input$tempmed, input$humid, input$sun, input$day,
                             input$mes))
      
    })
    
    # Use model on input data
    output$prediction1 <- reactive({
      data <- (inputData())
      data10    <- data[1]
      data11    <- data[2]
      data12    <- data[3]
      ...
      ##qx[1,19] <- as.caracter(quant19)
      ##qx[1,20] <- as.caracter(quant20)
      ##qx[1,21] <- as.caracter(quant21)
      ##qx[1,22] <- as.caracter(quant22)
      ##qx[1,23] <- as.caracter(quant23)
      ##qx[1,24] <- as.caracter(quant24)
      ##qx[1,25] <- as.caracter(quant25)
      ##qx[1,26] <- as.caracter(quant26)
      ##qx[1,27] <- (quant27)
      
      value1 <- c(quant19," - Desserts -( ",quant20," - Pizzas )- ( ",quant21," - Beverage )-( ",quant22," - Sfiha - ",quant23," - Snack - ", quant24," - Pastas - ", quant25," - Dishes - ",quant26," - Savory - ",quant27," - Salads -")
      ...
      ## df_1 <- data_frame(name = c("Desserts",  "Pizzas", "Beverage", "Sfihas", "Snack","Pastas", "Dishes", "Savory","Salads"),
      ##                  value1 = c(quant19,quant20,quant21,quant22,quant23,quant24,quant25,quant26,quant27),
      ##                   )

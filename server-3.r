...
shinyServer(
  function(input,output){
    ...
    print(df_5)
    library(ggplot2)
    output$myhist <- renderPlot({ggplot(data=data2x,x= data2x$Item,y=data2x$Quant, aes(Item, Quant),fill=time) + geom_bar(colour="blue",stat="identity")})
    output$view   <- renderTable({df_5()})
    ##output$hist <-renderPlot({df_5()})
